...
public class Main {
    ...
    private static void lifeCycle() {
        ...
        while (true) {
            ...
            if (inpCommand != null) {
                try {
                    if (inpCommand.getClass().equals(HistoryCmd.class)) { //если история
                        ...
                        if (!ret.isEmpty()) {
                            for(String str: ret){
                                ...
                                if(str.contains("зарегестрирован") || str.contains("пожаловать"))
                                    curClient.logIn(curClient);
                            }
                            if (!ret.get(0).startsWith("Ошибка")) //ошибкам не место в истории
                                CommandHistory.add(inpCommand);
                        }
                    }
    ...
    private static void connect(String ip) {
        ...
        do {
            try {
                ...
                do {
                    try {
                        clientSocket = new Socket(ip, 4006); //конектимся
                    } catch (IOException e) {
                        clientSocket = null;
                        Thread.sleep(1000);
                    }
