...
public final class Receiver {
    ...
    private static String movieToSqlValue(int id, Movie movie, Client owner) {
        ...
        String strGenre = (movie.getGenre() == null) ? "null" : "'" + movie.getGenre().name() + "'";
        String strRating = (movie.getMpaaRating() == null) ? "null" : "'" + movie.getMpaaRating().name() + "'";
        ...
        sql = String.format("(%s, '%s', '%s', ('%s', %s, '%s', (%d, %d, %d, '%s')), %s, %s, " +
                ...
                screenWriter.getNationality().name(), loc.getX(), loc.getY(), loc.getZ(), loc.getName(), strGenre,
    ...
    private static ArrayList<Movie> sqlToMovie(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            movie.setOwnerLogin(resultSet.getString("owner"));
            output.add(movie);
        }
        resultSet.beforeFirst();
        return output;
    ...
    public static void registration(Client owner, Client newUser) throws SQLException {
        if (findUser(newUser) != null) {
            output.add("Ошибка: пользователь с таким логином уже существует");
        } else {
    ...
    public static void clearCollection(Client owner) throws SQLException {
        ...
        if (res != null) {
            ...
            syncCollection();
            AnswerTask.massSync(getMoviesFromDB());
        } else
    ...
    public static void update(ArrayList<Movie> updatedMovies, Client owner) throws SQLException {
        ArrayList<Movie> old = getMoviesFromDB();
        updatedMovies.forEach(movie -> {
            if (!(old.indexOf(movie) > 0)) {
                String update = "UPDATE movies SET (id, title, creationdate, screenwriter, genre, mpaarating, boxoffice," +
                        "oscars, coordinates, owner) = " + movieToSqlValue(movie.getId(), movie, owner) + " WHERE id = " + movie.getId() +
                        " AND owner = '" + owner.getLogin() + "';";
                try {
                    int i = statement.executeUpdate(update);
                    if (i == 1) {
                        syncCollection();
                        output.add("Элемент с id = " + movie.getId() + " был обновлён");
                    }
